...
#pragma mark -
#pragma mark User_timeLine

- (void)getUserTimeLineUserID:(long long)userID withSinceID:(long long)sinceID 
				   maxiumumID:(long long)maxID statringAtPage:(int)page count:(int)count
{
	// TODO: Maybe should be NO
	needAuth = NO;
...
- (void)authError
{
    self.errorMessage = @"认证失败"; // Authentication Failed
    self.errorDetail  = @"用户名或密码错误"; //"Wrong username/Email and password combination.";    
...
- (void)URLConnectionDidFailWithError:(NSError*)error
{
    ...
    if (error.code ==  NSURLErrorUserCancelledAuthentication) {
        ...
        self.errorMessage = @"网络连接失败"; //"Connection Failed";
...
- (void)URLConnectionDidFinishLoading:(NSString*)content
{
    switch (statusCode) {
        ...
        {
            hasError = true;
            self.errorMessage = @"服务端返回一个错误"; //"Server responded with an error";
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        if (msg) {
            ...
            hasError = true;
            self.errorMessage = @"微博服务端出错";//"Weibo Server Error";
