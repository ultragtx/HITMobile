...
- (void)encodeWithCoder:(NSCoder *)encoder {
	[encoder encodeInt64:statusId forKey:[NSString stringWithFormat:@"Status_statusId_%d", encodeCount]];
	[encoder encodeInt32:createdAt forKey:[NSString stringWithFormat:@"Status_createdAt_%d", encodeCount]];
	[encoder encodeObject:text forKey:[NSString stringWithFormat:@"Status_text_%d", encodeCount]];
	[encoder encodeObject:source forKey:[NSString stringWithFormat:@"Status_source_%d", encodeCount]];
	[encoder encodeInt64:inReplyToStatusId forKey:[NSString stringWithFormat:@"Status_inReplyToStatusId_%d", encodeCount]];
	[encoder encodeInt:inReplyToUserId forKey:[NSString stringWithFormat:@"Status_inReplyToUserId_%d", encodeCount]];
	[encoder encodeObject:inReplyToScreenName forKey:[NSString stringWithFormat:@"Status_inReplyToScreenName_%d", encodeCount]];
	[encoder encodeObject:thumbnailPic forKey:[NSString stringWithFormat:@"Status_thumbnailPic_%d", encodeCount]];
	[encoder encodeObject:bmiddlePic forKey:[NSString stringWithFormat:@"Status_bmiddlePic_%d", encodeCount]];
	[encoder encodeObject:originalPic forKey:[NSString stringWithFormat:@"Status_originalPic_%d", encodeCount]];
	if (user) {
		[user encodeWithCoder:encoder];
	}
	encodeCount++;
	if (retweetedStatus) {
		[retweetedStatus encodeWithCoder:encoder];
	}
}

- (id)initWithCoder:(NSCoder *)decoder {
	if (self = [super init]) {
		self.source = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_source_%d", decodeCount]];
		if (!source) {
			return self;
		}
		self.statusId = [decoder decodeInt64ForKey:[NSString stringWithFormat:@"Status_statusId_%d", decodeCount]];
		self.createdAt = [decoder decodeInt32ForKey:[NSString stringWithFormat:@"Status_createdAt_%d", decodeCount]];
		self.text = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_text_%d", decodeCount]];
		
		self.inReplyToStatusId = [decoder decodeInt64ForKey:[NSString stringWithFormat:@"Status_inReplyToStatusId_%d", decodeCount]];
		self.inReplyToUserId = [decoder decodeIntForKey:[NSString stringWithFormat:@"Status_inReplyToUserId_%d", decodeCount]];
		self.inReplyToScreenName = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_inReplyToScreenName_%d", decodeCount]];
		self.thumbnailPic = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_thumbnailPic_%d", decodeCount]];
		self.bmiddlePic = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_bmiddlePic_%d", decodeCount]];
		self.originalPic = [decoder decodeObjectForKey:[NSString stringWithFormat:@"Status_originalPic_%d", encodeCount]];
		User *tempUser = [[User alloc] initWithCoder:decoder];
		self.user = tempUser;
		[tempUser release];
		decodeCount++;
		// in case of leak
		Status *tempRetweetStatus = [[Status alloc] initWithCoder:decoder];
		if (!tempRetweetStatus.source) {
			[tempRetweetStatus release];
			self.retweetedStatus = nil;
		}
		else {
			self.retweetedStatus = tempRetweetStatus;
			[tempRetweetStatus release];
		}
	}
	return self;
...
#pragma mark -
#pragma mark AssistantEncode

+ (void)setDecodeCount:(int)count {
	decodeCount = count;
}

+ (void)setEncodeCount:(int)count {
	encodeCount = count;
}
